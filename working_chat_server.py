#!/usr/bin/env python3
"""
Простой веб-сервер для рабочей автономной системы
"""
...
from working_autonomous_system import get_working_system
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/mentor/working_chat_server.log'),
...
app = FastAPI(title="Working Autonomous Multi-AI System", version="1.0.0")
...
async def startup_event():
    ...
    try:
        logger.info("🚀 Запуск рабочего веб-сервера...")
        working_system = get_working_system()
        
        # Запускаем рабочую систему
        if await working_system.start():
...
async def shutdown_event():
    ...
    global working_system
    try:
        logger.info("🛑 Остановка рабочего веб-сервера...")
        if working_system:
            working_system.stop()
        logger.info("✅ Рабочий веб-сервер остановлен")
    except Exception as e:
...
async def root():
    ...
    <head>
        ...
        <title>Working Autonomous Multi-AI System</title>
        <style>
            body {
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🤖 Working Autonomous Multi-AI System</h1>
                <p>Простая и надежная автономная система</p>
            </div>
            
            <div class="chat-container">
                <div class="chat-messages" id="chatMessages">
                    <div class="message agent-message">
                        <strong>Система:</strong> Добро пожаловать в рабочую автономную Multi-AI систему! Агенты работают автоматически.
                    </div>
                </div>
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if working_system:
                status = working_system.get_system_status()
...
async def get_agents():
    ...
    try:
        ...
        agents = []
        for agent_type, agent in working_system.agents.items():
            agents.append({
                "id": agent["id"],
                "name": agent["name"],
                "type": agent_type,
